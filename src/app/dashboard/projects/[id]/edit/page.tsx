...
export default function EditProjectPage() {
  ...
  const fetchProject = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch project:", err.message || err);
    } finally {
      setIsLoading(false);
    }
  }, [id, supabase]);
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {existingImages.map((img) => (
                    <div key={img.id} className="relative aspect-video group rounded-2xl overflow-hidden border border-gray-100 dark:border-white/10 transition-all hover:shadow-2xl">
                      <Image src={img.image_url} alt="Gallery" fill unoptimized className="object-cover group-hover:scale-110 transition-transform duration-700" />
